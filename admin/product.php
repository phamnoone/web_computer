...
if (isset($_POST['btnNew'])) {
	...
	if($_POST['chk']!=''){
		foreach ($_POST['chk'] as $id){
			...
			if ($pro){
				if (countRecord("tbl_product","product_id=".$pro['id']." and new != 0 ") <= 0){
					$result = mysql_query("update tbl_product set new = '1' where id = '".$pro['id']."'",$conn);
					if ($result){$cnt++;}
				}
			}
		}
...
if (isset($_POST['btnKm'])) {
	...
	if($_POST['chk']!=''){
		foreach ($_POST['chk'] as $id){
			$pro = getRecord($tableConfig, 'id='.$id);
			if ($pro){
				if (countRecord("tbl_product","id=".$pro['id']." and km > 0") <= 0){
					$result = mysql_query("update tbl_product set km = '10' where id = '".$pro['id']."'",$conn);
					if ($result){$cnt++;}
				}
			}
		}
...
<table cellspacing="0" cellpadding="0" width="100%">
	<tr>
		...
		<td align="right" class="smallfont">
			...
			<input type="button" value="Lọc sản phẩm" class="button" onClick="window.location='./?act=<?php echo $actConfig ?>&cat='+ddCat.value">
		</td>
	</tr>
</table>
...
<input type="button" value="Thêm mới" name="btnNew" onClick="window.location='./?act=<?php echo $actConfig?>_m&page=<?php echo $_REQUEST['page']?>&cat=<?php echo $_REQUEST['cat']?>'" class="button">
	  ...
	  <td align="right">
	 		<input type="submit" value="Sản phẩm khuyến mại" name="btnKm" class="button">
