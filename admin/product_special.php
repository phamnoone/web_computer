...
<?php $pageindex = createPage(countRecord($tableConfig,"id in ( select product_id from tbl_order_detail group by product_id ORDER BY COUNT(quantity))"),"./?act=".$actConfig."&cat=".$_REQUEST['cat']."&page=",$MAXPAGE,$page)?>
...
$sql="select * from tbl_product where id in ( select product_id from tbl_order_detail group by product_id ORDER BY COUNT(quantity)) limit 10 ";
